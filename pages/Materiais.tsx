      ...
      .then(async dataa => {
        for (const dz of dataa) {
          const data2 = await MaterialDeAula(dz.idDiario).catch(() => {
            console.log(
              'Falha ao obter material de aula para a disciplina: ' +
                dz.descricao,
            );
          });
      ...
      <SafeAreaView style={{padding: 20}}>
        {!dontshowagain ? (
          <Banner
            ...
            ATENÇÃO! O app não verifica materiais automaticamente. Arraste para
            baixo para verificar se há algum novo material.
          </Banner>
        ) : null}
        ...
        {data.length > 0 ? (
          data.map(diario => (
            <MaterialAula
              ...
              diario={diario}
              show={showDialog}
              saved={MMKV.getString(`materiais.${diario.idDiario}`) || '[]'}
            />
          ))
        ) : (
          <View style={styles.centered}>
            <Text variant="titleLarge" style={{textAlign: 'center'}}>
              {'\n'}Nenhum material. Role para baixo para atualizar
            </Text>
          </View>
        )}
        <Text>{'\n\n\n'}</Text>
