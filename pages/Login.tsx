...
export default function Entrar({navigation}): React.JSX.Element {
  ...
  const openLink = () => {
    Linking.openURL(
      'https://github.com/kkauaon/qaluno/releases/latest/download/qaluno.apk',
    ).catch(() =>
    ...
    if (matr && senha) {
      ...
      Login(matr, senha)
        .then(async data => {
          ...
          await HorarioIndividual(sem.split('.')[0], sem.split('.')[1])
            .then(data => {
  ...
  return (
    <SafeAreaView
        ...
        <Dialog
          ...
          <Dialog.Content>
            <Text variant="bodyMedium">
              ...
              continuar usando o aplicativo.{'\n\n'}Ao clicar em baixar, o
              download deve começar automaticamente, caso não, copie o link e
              insira manualmente no seu navegador.
            </Text>
          </Dialog.Content>
          <Dialog.Actions>
      ...
      <Image
        source={
          colorScheme == 'dark'
            ? require('../img/app_icon.png')
            : require('../img/app_icon_black.png')
        }
        style={{width: 128, height: 128}}
      />
      <Text variant="displayMedium">QAluno</Text>
      <Text variant="titleMedium">
        Aplicativo não oficial do Q-Acadêmico para IFCE
      </Text>
      <TextInput
        label="Matrícula"
        style={{width: '80%'}}
      ...
      <Button
        ...
        mode="contained"
        style={{width: '80%'}}
        labelStyle={{fontSize: 20, lineHeight: 35}}>
        Entrar
      </Button>
      <Text
